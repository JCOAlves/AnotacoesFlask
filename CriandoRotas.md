# Rotas na aplicação Flask
Rotas são essenciais para em uma aplicação, pois definem as requisições do usuário. Você pode criar múltiplas rotas em sua aplicação Flask. 

## 1. Criando Rotas
Para isso, modifique `app.py` para incluir uma nova rota:
...
## 2. Utilizando Variáveis nas Rotas
...
Para declarar variáveis em uma rota adicione `<nome_da_variavel>` a ela. O Flask automaticamente passa o valor capturado dessa parte da URL como um argumento para a função de visualização associada.
...
## 3. Tipos de dados na Rotas
Por padrão, as variáveis capturadas são strings. No entanto, o Flask oferece **conversores** que permitem especificar o tipo de dado que você espera para a variável na URL. Isso é útil para validação e para que o Python já receba o tipo correto (inteiro, float, etc.). Você usa os conversores adicionando `:tipo` após o nome da variável: `<nome_da_variavel:tipo>`.
